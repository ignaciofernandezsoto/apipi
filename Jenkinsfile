...
pipeline {
    ...
    stages {
        ...
        stage('Create new container name') {
            steps {
                script {
                    ...
                    def new_full_container_name = sh script:'echo "${APIPI_FULL_IMAGE_NAME}_' + trimmed_current_date + '_' + trimmed_random_string + '"', returnStdout: true
        ...
        stage('Run new docker container') {
            steps {
                sh 'docker run --network="host" --name ${APIPI_SHORT_IMAGE_NAME} -d --restart unless-stopped ' + NEW_FULL_CONTAINER_NAME
            }
        }
    }
}